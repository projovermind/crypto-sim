...
type SidebarTab = 'account' | 'users' | 'teledit'

const SIDEBAR_ITEMS: { key: SidebarTab; label: string }[] = [
  { key: 'account', label: '내정보' },
...
export default function AdminPage() {
  ...
  const filtered = userFilter === 'pending'
                ...
                {(['pending', 'all'] as const).map(f => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded text-xs font-medium transition-colors ${
                      userFilter === f ? 'bg-binance-yellow text-binance-bg' : 'text-binance-text-dim hover:text-binance-text bg-binance-card'
                    }`}
                  >
